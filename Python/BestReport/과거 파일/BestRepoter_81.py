...
def fileEdit():
    ...
    filewb=load_workbook(filename,data_only=True)  #버튼으로 선택한 엑셀 파일 불러오기

    for ws, i in zip(filewb.sheetnames, range(2,len(filewb.sheetnames))):
        print(filewb[ws])
        filewb["Sheet"].cell(row=1,column=1).value="dma"
        if (ws == "Sheet"):
            #jira 추출한거 넣어주기
            ...
            for r in filewb[ws].rows:               #워크시트의 열의 갯수만큼 반복
                ####함수 넣기#####
                if (filewb[ws].cell(row=count, column=7)=="JIRA 등록"):     #워크시트의 처리 여부가 jira 등록일 경우 
                    filewb[ws].cell(row=count,column=3).value=f"=VLOOKUP(B{count},Sheet!B:C,2,0)"   # jira 제목 찾기

                elif (filewb[ws].cell(row=count, column=7)=="NextUpdate"):  #워크시트의 처리 여부가 넥업일 경우
                    filewb[ws].cell(row=count,column=3).value=f"=VLOOKUP(B{count},Sheet!B:C,2,0)"   # jira 제목 찾기
...
fileEditBtn=tkinter.Button(subLabelFrame1,text="파일 편집", width="30", command=fileEdit)
