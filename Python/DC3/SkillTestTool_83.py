...
def stat_cal_test(jobBtn):
    ...
    #스공 계산 공식
    if(jobBtn=="nomal"):
        resultMAXPAD=(mainstat*4+substat1+substat2)/100*(pad*weapon_constant) #(주스텟*4+부스텟1+부스텟2)/100*(공/마*무기상수)
        resultMINPAD=round(resultMAXPAD*deviation,0) #근접공격 데미지 편차 적용...
        result_stat_minpad.config(text=resultMINPAD)    #최소 스공
        result_stat_maxpad.config(text=round(resultMAXPAD,0)) #최대 스공
    elif(jobBtn=="xenon"):
        resultMAXPAD=((mainstat+substat1+substat2)*3.5)/100*(pad*1.3) #((주스텟+부스텟1+부스텟2)*3.5/100*(공/마*무기상수))
        resultMINPAD=round(resultMAXPAD*deviation,0)
        result_stat_minpad.config(text=resultMINPAD)
        result_stat_maxpad.config(text=round(resultMAXPAD,0))
    elif(jobBtn=="demon"):
        resultMAXPAD=4*((pureHP/14)+(addHP/17.5))+substat1/100*(pad*1.3) #4*((순수체력/14)+(증가체력/17.5))+부스텟(STR)/100*(공/마*무기상수)
        resultMINPAD=round(resultMAXPAD*deviation,0)
        result_stat_minpad.config(text=resultMINPAD)
        result_stat_maxpad.config(text=round(resultMAXPAD,0))
    elif(jobBtn=="dual"):
        ...
        result_stat_maxpad.config(text="error")
